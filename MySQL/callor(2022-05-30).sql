...
-- 가정먼저 어떤 DB를 사용할 것인지 알려줘야 한다.
USE mydb;

DROP TABLE tbl_test;
-- Table 을 생성할때 칼럼의 이름은 사전에 있는 키워드는 가급적
-- 사용하지 않는 것이 좋다
-- 사전에 있는 키워드를 사용할때는 접두사를 부착하여 사용 하는 것이 좋다.
...
MySQL을 사용하는데 
MySQL에서 사용하는 방식이 DBMS의 업계표준으로
정착이 된 상태

대부분의 DBMS 들은 먼저 Database 를 생성하고
사용자가 Database에 접근할 수 있는 권한을 부여하여
사용자는 Database를 사용하겠다 라고 선언을 한 후
DB 관련된 명령을 사용한다.
*/

insert into tbl_test(t_name) values ('홍길동');

select * from tbl_test;
-- 다중데이터 insert 하기 
insert into tbl_test(t_name) values('이몽룡') , ('성춘향'), ('장영자'), ('서시'), ('장희빈');
select * from tbl_test;
/*
tbl_test의 ID 칼럼은 auto_increment 로 설정 되었기 때문에
데이터를 insert 할때만다 자동으로 sequence 값이 생성되어
추가된다
5.x MySQL 에서는 auto_increment 칼럼에 이슈가 있었다
5.x 숫자 type은 주로 int형을 사용하는데 int형은 정수11자리이다.
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 insert가 불가능해지는
문제가 있었다.
5.7 버전 이후에는 bigint형 타입이 도입되어 문제를 해결하였다.
DB table 을 만들때 auto_increment 칼럼에는 반드시 bigint형을 지정하자
...
pk 아닌경우는 반드시 not null 과 unique를 같이 설정한다.
pk 또는 not null and nuique 설정이 되지 않았을 경우
table 생성에서 오류가 난다.
*/
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','홍길동','컴퓨터공학과',1,'010-111-1111','익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0102','홍길동','컴퓨터공학과',3,'010-111-1112','서울시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0104','성춘향','컴퓨터공학과',2,'010-111-1113','익산시');
